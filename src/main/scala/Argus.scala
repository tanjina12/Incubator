  ...
  def Intent(apk: ApkGlobal): Unit = {
    ...
    candidate.foreach { case (ctx, s) =>
      ...
      if (intent.head.componentNames.nonEmpty) {
        ...
        println(s"NO component link. Its likely ann action ${intent}")
      }
    }
...
//    generateDotFile(cg, apk)
    generateDotFileSmall(cg, apk)
  }
  ...
  def generateDotFile(cg: CallGraph, apk: ApkGlobal): Unit = {
  ...
  def generateDotFileSmall(cg: CallGraph, apk: ApkGlobal): Unit = {
